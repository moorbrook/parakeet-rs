...
/// How long to wait before re-checking whether the system consent alert
/// appeared. The alert takes focus away from this app, so "app still active
/// + still not granted" after this window means no prompt is coming.
const INPUT_MONITORING_RECHECK_DELAY: std::time::Duration = std::time::Duration::from_millis(1500);
...
/// a first request macOS shows the consent alert and the call still returns
/// false while the user has not answered, so the immediate return alone
/// cannot tell a live prompt from a suppressed one. The answer comes from a
/// delayed re-check: a consent alert (or a trip to System Settings) takes
/// focus away from this app, while a suppressed request — denied, or the
/// stored decision went stale because the app was rebuilt under a different
/// signature — leaves the app active and the grant missing.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum InputMonitoringRecheckOutcome {
    ...
    /// rebuilt under a different signature. Open the Input Monitoring pane
...
fn input_monitoring_fallback_warning() -> &'static str {
    "no Input Monitoring consent prompt appeared and the grant is still not \
     in place (denied, or the stored decision went stale after a rebuild); \
     opening Privacy & Security > Input Monitoring"
}
...
mod tests {
    ...
    #[test]
    fn input_monitoring_recheck_distinguishes_a_live_prompt_from_a_suppressed_one() {
    ...
    fn input_monitoring_fallback_warning_names_the_pane_and_the_recovery_route() {
        ...
        assert!(warning.contains("still not"));
        assert!(warning.contains("no Input Monitoring consent prompt"));
    }
}
