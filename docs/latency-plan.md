...
| 4 | 5 s with-polish p50 ≤ 1.0 s | ✅ **MET on the polish eval set (amended 2026-09, Kata 0tpp)** | The 1225 ms previously recorded here was one hardcoded worst-case transcript, not a percentile — 55 output tokens at 43.4 tok/s, which is the configuration's structural bound. Measured over the 26-item [`bench/polish/eval.json`](../bench/polish/eval.json), the shipping Qwen 3.5 4B Q6_K path is **444 ms p50 / 1219 ms p95**, and that same run reproduces **1209 ms** on the historical transcript alone. Both numbers are published; the eval set's composition is documented in [`../bench/README.md`](../bench/README.md) so the blended figure can be re-weighted. See [ADR-0030](./ADR.md#0030--polish-latency-the-target-was-met-the-measurement-was-not). |
